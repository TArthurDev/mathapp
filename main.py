...
if not os.path.exists(PROBLEMS_FILE):
    sample_problems = [
        {"id": "prob1", "title": "Prime Time", "concept": "Number Theory", "question": "How many positive integers less than 100 have exactly three positive divisors?", "official_answer": "4", "rubric": "Squares of primes. Primes <10: 2,3,5,7 => 4,9,25,49. Ans: 4.", "image_url": "", "lesson_link": ""},
        {"id": "prob2", "title": "Arranging Letters", "concept": "Combinatorics", "question": "In how many ways can the letters of the word 'MATH' be arranged if the vowels must be together?", "official_answer": "24", "rubric": "There is only 1 vowel ('A'). It is always together with itself.", "image_url": "", "lesson_link": ""}
    ]
    save_json(PROBLEMS_FILE, sample_problems)
else:
    existing_problems = sanitize_problems(load_json(PROBLEMS_FILE, []))
    if existing_problems != load_json(PROBLEMS_FILE, []):
        save_json(PROBLEMS_FILE, existing_problems)
...
async def submit_solution(request: SubmitRequest):
    ...
    print("DEBUG - Loaded Problem Statement:", problem.get("question") if problem else None)
    print("DEBUG - Loaded Rubric:", problem.get("rubric") if problem else None)
    ...
    fallback = {
        ...
        "resources_used": "",
        "url": problem.get("url", "") if problem else ""
    }
    ...
    try:
        ...
        system_prompt = (
            ...
            "CRITICAL RULE: You must accept the provided Rubric and Reference Solution as absolute, objective truth for this universe, even if it contradicts the laws of mathematics, logic, or reality in the real world. If the rubric states an incorrect mathematical fact (e.g., 1 + 1 = 3) and the student follows that fact, you must award full points according to the rubric guidelines. Do not apply real-world corrections. "
        ...
        lesson_link = problem.get("lesson_link", "") if problem else ""
        problem_url = problem.get("url", "") if problem else ""
    ...
    correctness_score = float(feedback.get("correctness_score", 0.0))
    if correctness_score >= 7.0:
        current_streak += 1
    else:
        current_streak = 0
    ...
    feedback.setdefault("teacher_solution", "")
    feedback.setdefault("tutor_hint", "Please try again later.")
...
def get_student_submissions(student_id: str):
    return student_submissions.get(student_id.lower().strip(), [])

@app.post("/api/admin/reset")
def reset_student_data():
    global student_submissions, user_streak, all_club_submissions
    student_submissions = {}
    user_streak = 0
    all_club_submissions = []
    save_json(HISTORY_FILE, [])
