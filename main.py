...
# A student "completes" a problem once they get it right, or once they've used this
# many attempts — must match MAX_ATTEMPTS in the frontend.
MAX_ATTEMPTS = 2
PASSING_SCORE = 7.0
    ...
    sample_problems = [
        {
            ...
            "difficulty": "Medium", "image_url": "", "lesson_link": "", "url": ""
        },
        {
            ...
            "difficulty": "Easy", "image_url": "", "lesson_link": "", "url": ""
        }
    ]
...
def add_problem(problem: ProblemCreate):
    ...
    new_problem = {
        ...
        "rubric": problem.reference_solution, "difficulty": problem.difficulty or "",
        "image_url": problem.image_url or "",
...
async def submit_solution(request: SubmitRequest):
    ...
    if feedback["correctness_score"] >= PASSING_SCORE:
        current_streak += 1
    else:
        current_streak = 0
    ...
    submission_entry = {
        ...
        "is_pasted": bool(request.is_pasted),
        "timestamp": time.time(),
        **feedback
    }
...
@app.get("/api/student/{student_code}/progress", response_model=ProgressResponse)
def get_student_progress(student_code: str):
    """Tells the frontend which problems this student has already finished, so a
    page refresh / re-login resumes where they left off instead of starting over."""
    student_code = student_code.lower().strip()
    history = load_json(HISTORY_FILE, [])
    student_history = [e for e in history if e.get("student_code") == student_code]

    best_score_by_problem: Dict[str, float] = {}
    attempts: Dict[str, int] = {}
    for entry in student_history:
        pid = entry.get("problem_id")
        if not pid:
        ...
        score = float(entry.get("correctness_score", 0) or 0)
        if pid not in best_score_by_problem or score > best_score_by_problem[pid]:
            best_score_by_problem[pid] = score

    completed = [
        pid for pid, cnt in attempts.items()
        if best_score_by_problem.get(pid, 0) >= PASSING_SCORE or cnt >= MAX_ATTEMPTS
    ]
    correct = [pid for pid, score in best_score_by_problem.items() if score >= PASSING_SCORE]

    return ProgressResponse(completed_problem_ids=completed, correct_problem_ids=correct, attempts=attempts)
...
def get_leaderboard():
    ...
    for student_code in students:
        ...
        avg = sum(
            (e.get("correctness_score", 0) + e.get("explanation_score", 0) + e.get("rigor_score", 0)) / 3.0
            for e in student_history
        ) / total
        entries.append(LeaderboardEntry(student_code=student_code, avg_score=round(avg, 2), total_submissions=total))
...
def export_csv():
    ...
    writer.writerow([
        "student_code", "problem_id", "concept", "difficulty", "exact_answer",
        "correctness_score", "explanation_score", "rigor_score", "is_pasted", "timestamp"
    ])
    for entry in history:
        ...
        writer.writerow([
            ...
            prob.get("concept", ""),
            prob.get("difficulty", ""),
            entry.get("exact_answer", ""),
            entry.get("correctness_score", ""),
            entry.get("explanation_score", ""),
            entry.get("rigor_score", ""),
            entry.get("is_pasted", False),
            ts_str
        ])
    ...
    return Response(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=mathclub_performance.csv"}
    )
